    ...
    register: async (req, res, next) => {
        try {
            ...
            if (userInserted) {
                ...
                const payload = {
                    userId: userInserted.id, // Assurez-vous que `id` est correctement retourné par `authService.insert`
                    login: userInserted.login,
                };
                const token = jwt.sign(payload, process.env.JWT_SECRET, {
                    expiresIn: "2d",
                });

                // Vous pouvez choisir de stocker ou non ce token JWT dans votre base de données,
                // dépendant de votre logique d'application.
